...
export default function CeoReports() {
  ...
  const recordRows = selectedRecord ? [selectedRecord] : employees;
  ...
  return <div className="min-h-full bg-[#f7f9f8] px-4 py-6 text-[#17251f] sm:px-6 lg:px-8">
    <div className="mx-auto max-w-[1560px]">
      ...
      {activeTab === 'records' && <Card className="overflow-hidden"><div className="flex flex-col gap-4 border-b border-[#edf1ef] p-5 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-lg font-bold">Employee Performance Records</h2><p className="mt-1 text-xs text-[#929d98]">All employee records for the selected period.</p></div><div className="flex flex-col gap-2 sm:flex-row"><select value={recordUserId ?? ''} onChange={event => setRecordUserId(event.target.value ? Number(event.target.value) : null)} className="h-11 min-w-56 rounded-xl border border-[#dfe6e3] bg-white px-3 text-sm outline-none focus:border-emerald-500"><option value="">All employees</option>{employees.map(employee => <option key={employee.user.id} value={employee.user.id}>{employee.user.name} — {employee.user.employee_id || employee.role}</option>)}</select><button disabled={!recordRows.length} onClick={() => exportPdf(recordRows, selectedRecord ? `performance-${selectedRecord.user.name.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}` : 'performance-all-employees')} className="h-11 rounded-xl bg-emerald-600 px-4 text-sm font-bold text-white hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-40">Export {selectedRecord ? 'selected' : 'all'}</button></div></div><div className="overflow-x-auto"><table className="w-full min-w-[880px] text-left"><thead className="bg-[#fafbfa] text-[11px] font-semibold text-[#8b9691]"><tr><th className="px-6 py-4">Employee</th><th>Job title</th><th className="text-center">Attendance</th><th className="text-center">Working hours</th><th className="text-center">Tickets</th><th className="text-center">Worklogs</th><th className="px-6 text-center">Index</th></tr></thead><tbody className="divide-y divide-[#edf1ef]">{recordRows.map((employee,index) => <tr key={employee.user.id} className="hover:bg-[#fbfdfc]"><td className="px-6 py-4"><div className="flex items-center gap-3"><Avatar name={employee.user.name} index={index}/><span><b className="block text-sm">{employee.user.name}</b><small className="text-[#9aa49f]">{employee.user.employee_id || `EMP-${employee.user.id}`}</small></span></div></td><td><b className="text-sm text-[#45534e]">{employee.role}</b><small className="mt-1 block text-[#9aa49f]">{employee.department}</small></td><td className="text-center font-semibold">{employee.attendance}%</td><td className="text-center font-semibold">{employee.workingHours.toFixed(1)}h</td><td className="text-center font-semibold">{employee.completedTickets}/{employee.assignedTickets}</td><td className="text-center font-semibold">{employee.ticketHours.toFixed(1)}h</td><td className="px-6 text-center font-extrabold text-emerald-700">{employee.score}/100</td></tr>)}{!recordRows.length && <tr><td colSpan={7} className="px-6 py-12 text-center text-sm text-[#929d98]">No records found for this period.</td></tr>}</tbody></table></div></Card>}
