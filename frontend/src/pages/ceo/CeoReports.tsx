...
function Avatar({ name, index }: { name: string; index: number }) {
  return <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full text-[11px] font-bold text-white ring-4 ring-emerald-50" style={{ background: `linear-gradient(145deg,${palette[index % palette.length]},#0b5b50)` }}>{initials(name)}</span>;
}
...
function PeriodSelect({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return <label className="relative inline-flex shrink-0 items-center">
    <select value={value} onChange={event => onChange(event.target.value)} className="h-11 min-w-[148px] cursor-pointer appearance-none rounded-[14px] border-0 bg-[#eaf6d9] py-0 pl-5 pr-11 text-sm font-semibold text-[#385d4e] outline-none transition hover:bg-[#e1f1ca] focus:ring-2 focus:ring-[#74c5b2]/40">
      <option>This Month</option>
      <option>Last Month</option>
    </select>
    <svg className="pointer-events-none absolute right-4 h-4 w-4 text-[#385d4e]" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="m5 7.5 5 5 5-5" /></svg>
  </label>;
}
...
export default function CeoReports() {
  ...
  const categories = [
    { label: 'Attendance Rate', value: `${avgAttendance}%`, percent: avgAttendance, color: '#42c7b0' },
  ...
  const sortLabel = (label: string, key: string) => <button onClick={() => changeSort(key)} className="inline-flex items-center gap-1 hover:text-[#176a59]">{label}<span>{sort.key === key ? (sort.direction === 'asc' ? '↑' : '↓') : '↕'}</span></button>;
  ...
  return <div className="min-h-full bg-[#f7f9f8] px-4 py-6 text-[#17251f] sm:px-6 lg:px-8">
    <div className="mx-auto max-w-[1560px]">
      <header className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div><h1 className="text-[26px] font-bold tracking-[-.035em]">Performance</h1><p className="mt-1 text-sm text-[#8a9691]"><span className="font-semibold text-[#52bda7]">Dashboard</span><span className="mx-2">/</span>Performance</p></div>
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <button onClick={exportPdf} className="h-12 rounded-2xl bg-[#eaf6d9] px-5 text-sm font-bold text-[#245849] hover:bg-[#dff0c3]">Export report</button>
        </div>
      </header>
          ...
          <div className="grid gap-5 lg:grid-cols-[minmax(330px,.9fr)_minmax(420px,1.3fr)]">
            <Card className="p-5 sm:p-6">
              ...
              <div className="flex h-64 items-end justify-around gap-3 border-b border-[#edf1ef] px-1">
                {departments.map(department => <div key={department.name} className="flex h-full min-w-0 flex-1 flex-col items-center justify-end" title={`${department.name}: ${department.attendance}% attendance, ${department.completion}% ticket completion`}><span className="mb-3 text-xs font-bold text-[#50605a]">{department.score}%</span><div className="flex h-[76%] w-full max-w-[48px] flex-col justify-end overflow-hidden rounded-t-lg bg-[#f2f4f3]"><span className="bg-[#dff1c7]" style={{ height: `${Math.max(0, 100 - department.score)}%` }}/><span className="bg-[#43bea6]" style={{ height: `${department.completion * .4}%` }}/><span className="bg-[#105d50]" style={{ height: `${department.attendance * .6}%` }}/></div><span className="mt-3 max-w-full truncate text-[11px] text-[#7c8984]">{department.name.split(' ')[0]}</span></div>)}
              ...
              <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-[11px] text-[#87938e]">{[['#105d50','Attendance (60%)'],['#43bea6','Tickets done (40%)'],['#dff1c7','Remaining']].map(([color,label]) => <span key={label} className="flex items-center gap-2"><i className="h-1.5 w-6 rounded-full" style={{ backgroundColor: color }}/>{label}</span>)}</div>
            </Card>
