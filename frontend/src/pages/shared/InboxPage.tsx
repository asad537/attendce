    ...
    <div className="relative z-10 flex flex-1 flex-col items-center justify-between px-6 py-12">
      ...
      {status === 'incoming'
        ? <div className="flex items-center gap-12">
            ...
            <button onClick={accept} className="flex flex-col items-center gap-2"><span className="grid h-16 w-16 place-items-center rounded-full bg-[#10b981] shadow-lg transition hover:bg-[#059669]">{isVideo ? <VideoIcon/> : <PhoneIcon/>}</span><small className="text-white/70">Accept</small></button>
          </div>
        : <div className="flex items-center gap-5">
            <button onClick={toggleMute} className={`grid h-14 w-14 place-items-center rounded-full text-xl transition ${muted ? 'bg-white text-[#0c241b]' : 'bg-white/15 hover:bg-white/25'}`} title={muted ? 'Unmute' : 'Mute'}>{muted ? '🔇' : '🎙'}</button>
            {isVideo && <button onClick={toggleCam} className={`grid h-14 w-14 place-items-center rounded-full transition ${camOff ? 'bg-white text-[#0c241b]' : 'bg-white/15 hover:bg-white/25'}`} title={camOff ? 'Turn camera on' : 'Turn camera off'}><VideoIcon/></button>}
            <button onClick={hangup} className="grid h-16 w-16 place-items-center rounded-full bg-[#e94141] shadow-lg transition hover:bg-[#d12f2f]" title="End call"><span className="rotate-[135deg]"><PhoneIcon/></span></button>
          </div>}
    </div>
  </div>;
}
...
export default function InboxPage() {
  ...
  return <div className="h-full min-h-[650px] bg-[#f0f2f5]">
    <div className="flex h-full min-h-[650px] overflow-hidden border-t-[3px] border-[#10b981] bg-white">
      <aside className={`${showPeople ? 'flex' : 'hidden'} w-full shrink-0 flex-col border-r border-[#e7ecea] sm:flex sm:w-80 xl:w-96`}>
        <header className="border-b border-[#e7ecea] bg-white px-3 py-3">
          <div className="flex items-center gap-3 px-1"><h1 className="text-xl font-bold">Chats</h1><button onClick={() => { setSearch(''); setFilter('new'); }} title="New chat" className="ml-auto grid h-9 w-9 place-items-center rounded-full text-xl text-[#546b63] hover:bg-[#edf2f0]">✎</button><button className="grid h-9 w-9 place-items-center rounded-full text-xl text-[#546b63] hover:bg-[#edf2f0]">⋮</button></div>
          <label className="relative mt-3 block"><svg className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#7f8c87]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="7"/><path d="m20 20-4-4"/></svg><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search" className="h-9 w-full rounded-lg border border-[#dfe5e2] bg-white pl-9 pr-3 text-sm outline-none focus:border-[#34d399]" /></label>
          <div className="mt-3 flex gap-2 overflow-x-auto">{([['all','All'],['unread','Unread'],['recent','Recent'],['new','New']] as const).map(([key,label]) => <button key={key} onClick={() => setFilter(key)} className={`rounded-full border px-4 py-1.5 text-xs font-semibold ${filter === key ? 'border-[#6ee7b7] bg-[#d1fae5] text-[#047857]' : 'border-[#dfe5e2] bg-white text-[#6d7874] hover:bg-[#f4f7f6]'}`}>{label}</button>)}</div>
        </header>

        <div className="flex-1 overflow-y-auto">{isLoading ? <div className="p-10 text-center text-sm text-[#84918c]">Loading people…</div> : visibleConversations.map((conversation, index) => <button key={conversation.user.id} onClick={() => chooseConversation(conversation)} className={`flex w-full gap-3 px-3 py-3 text-left transition hover:bg-[#f3f5f4] ${selectedUserId === conversation.user.id ? 'bg-[#f0f2f5]' : ''}`}><Avatar user={conversation.user} index={index}/><span className="min-w-0 flex-1 border-b border-[#edf1ef] pb-3"><span className="flex items-center gap-2"><b className="truncate text-sm text-[#1f2c28]">{conversation.user.name}</b><small className={`ml-auto shrink-0 text-[11px] ${conversation.unread_count ? 'text-[#059669]' : 'text-[#8a9691]'}`}>{timeLabel(conversation.last_message?.created_at)}</small></span><span className="mt-1 flex items-center gap-2"><small className={`truncate ${conversation.unread_count ? 'font-semibold text-[#33463f]' : 'text-[#87938e]'}`}>{conversation.last_message ? <>{conversation.last_message.sent_by_me && <DoubleCheck className="mr-0.5 h-[13px] w-[15px] text-[#53bdeb]"/>}{conversation.last_message.body}</> : <span className="italic">Start a conversation</span>}</small>{conversation.unread_count > 0 && <b className="ml-auto grid h-5 min-w-5 shrink-0 place-items-center rounded-full bg-[#10b981] px-1 text-[10px] text-white">{conversation.unread_count}</b>}</span></span></button>)}{!isLoading && !visibleConversations.length && <div className="p-10 text-center text-sm text-[#84918c]">No chats in this filter.</div>}</div>
      </aside>

      <main className={`${showPeople ? 'hidden' : 'flex'} min-w-0 flex-1 flex-col bg-[#efeae2] sm:flex`}>
        {selectedUserId && thread ? <>
          ...
          <div className="flex-1 overflow-y-auto bg-[#efe9df] bg-[radial-gradient(circle_at_25%_25%,rgba(255,255,255,.48)_0_3px,transparent_4px),radial-gradient(circle_at_75%_70%,rgba(255,255,255,.35)_0_5px,transparent_6px)] bg-[length:62px_62px] p-4 sm:px-7">{threadLoading ? <p className="text-center text-sm text-[#7f8c87]">Loading chat…</p> : groupedMessages.map(group => <div key={group.label}><div className="my-5 flex justify-center"><span className="rounded-lg bg-white/90 px-3 py-1.5 text-[11px] font-medium text-[#65736e] shadow-sm">{group.label}</span></div>{group.messages.map(item => { const mine = item.sender.id === me?.id; const att = item.attachment; return <div key={item.id} className={`group mb-1.5 flex ${mine ? 'justify-end' : 'justify-start'}`}><div className={`relative max-w-[85%] rounded-lg px-2 py-1.5 shadow-sm sm:max-w-[72%] ${mine ? 'rounded-tr-none bg-[#d1fae5]' : 'rounded-tl-none bg-white'}`}>{item.is_deleted ? <p className="flex items-center gap-1.5 py-0.5 pr-12 text-[13px] italic leading-5 text-[#8996a0]"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-3.5 w-3.5 shrink-0"><circle cx="12" cy="12" r="10"/><path d="M4.9 4.9l14.2 14.2"/></svg>This message was deleted</p> : <>{att && (att.is_image ? <a href={att.url} target="_blank" rel="noreferrer" className="mb-1 block"><img src={att.url} alt={att.name} className="max-h-72 max-w-[240px] rounded-md object-cover"/></a> : <a href={att.url} target="_blank" rel="noreferrer" download={att.name} className="mb-1 flex items-center gap-2 rounded-md bg-black/5 px-2.5 py-2 hover:bg-black/10"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[#10b981] text-white"><FileIcon/></span><span className="min-w-0"><b className="block max-w-[150px] truncate text-[12px] text-[#24342e]">{att.name}</b><small className="text-[10px] text-[#74817c]">{formatSize(att.size)}</small></span></a>)}{item.body && <p className="whitespace-pre-wrap break-words px-0.5 text-[13px] leading-5 text-[#24342e]">{item.body}</p>}</>}<span className="float-right ml-8 inline-flex items-center gap-1 pr-0.5 text-[9px] text-[#74817c]">{format(new Date(item.created_at), 'hh:mm a')}{mine && !item.is_deleted && <DoubleCheck className="ml-0.5 h-[13px] w-[15px] text-[#53bdeb]"/>}</span>{!item.is_deleted && <button onClick={() => setMenuFor(menuFor === item.id ? null : item.id)} title="Message options" className={`absolute right-1 top-1 z-10 h-6 w-6 place-items-center rounded-full text-[#54656f] shadow-sm backdrop-blur-sm ${att?.is_image ? 'bg-white/85' : mine ? 'bg-[#c8efbf]/90' : 'bg-[#f0f2f0]/90'} ${menuFor === item.id ? 'grid' : 'hidden group-hover:grid'}`}><ChevronDown/></button>}{menuFor === item.id && <><div className="fixed inset-0 z-0" onClick={() => setMenuFor(null)}/><div className={`absolute top-0 z-20 w-44 overflow-hidden rounded-lg border border-[#e2e6e4] bg-white py-1 text-left text-[13px] shadow-xl ${mine ? 'right-full mr-2' : 'left-full ml-2'}`}><button onClick={() => remove.mutate({ id: item.id })} disabled={remove.isPending} className="block w-full px-3 py-2 text-left text-[#33463f] hover:bg-[#f3f5f4]">Delete for me</button>{mine && <button onClick={() => remove.mutate({ id: item.id, scope: 'everyone' })} disabled={remove.isPending} className="block w-full px-3 py-2 text-left text-[#c0392b] hover:bg-[#fdecea]">Delete for everyone</button>}<button onClick={() => setMenuFor(null)} className="block w-full px-3 py-2 text-left text-[#7f8c87] hover:bg-[#f3f5f4]">Cancel</button></div></>}</div></div>; })}</div>)}{!threadLoading && !thread.messages.length && <div className="grid h-full place-items-center text-center"><span className="rounded-lg bg-[#fff6c9] px-5 py-3 text-xs text-[#65736e] shadow-sm">Messages are private between you and {thread.user.name}.</span></div>}<div ref={bottomRef}/></div>
          <footer className="shrink-0 bg-[#f0f2f5] px-3 py-2">{pendingFile && <div className="mb-2 flex items-center gap-2 rounded-lg bg-white px-3 py-2 shadow-sm">{pendingFile.type.startsWith('image/') ? <img src={URL.createObjectURL(pendingFile)} alt="" className="h-11 w-11 rounded object-cover"/> : <span className="grid h-11 w-11 shrink-0 place-items-center rounded bg-[#d1fae5] text-[#047857]"><FileIcon/></span>}<span className="min-w-0 flex-1"><b className="block truncate text-[13px] text-[#24342e]">{pendingFile.name}</b><small className="text-[11px] text-[#74817c]">{formatSize(pendingFile.size)}</small></span><button onClick={() => setPendingFile(null)} title="Remove" className="grid h-8 w-8 shrink-0 place-items-center rounded-full text-lg text-[#87938e] hover:bg-[#f0f2f5]">✕</button></div>}<div className="flex items-center gap-2"><input ref={fileInputRef} type="file" onChange={onPickFile} className="hidden"/><button onClick={() => fileInputRef.current?.click()} title="Attach" className="grid h-10 w-10 shrink-0 place-items-center rounded-full text-[#61716b] hover:bg-[#e2e6e4]"><ClipIcon/></button><textarea value={message} onChange={event => setMessage(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); submit(); } }} placeholder="Type a message" rows={1} className="max-h-28 min-h-10 flex-1 resize-none rounded-lg border-0 bg-white px-4 py-2.5 text-sm outline-none"/><button onClick={submit} disabled={(!message.trim() && !pendingFile) || send.isPending} className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#10b981] text-base text-white transition hover:bg-[#059669] disabled:bg-[#aab7b2]">➤</button></div></footer>
        </> : <div className="grid flex-1 place-items-center bg-[#f7faf9] p-8 text-center"><div><div className="mx-auto grid h-24 w-24 place-items-center rounded-full bg-[#d1fae5] text-5xl">💬</div><h2 className="mt-6 text-2xl font-bold">Workplace Chat</h2><p className="mt-2 max-w-sm text-sm leading-6 text-[#7f8c87]">Select any employee from the list and start a private conversation.</p><button onClick={() => setShowPeople(true)} className="mt-5 rounded-xl bg-[#10b981] px-5 py-2.5 text-sm font-semibold text-white sm:hidden">Choose employee</button></div></div>}
      </main>
    </div>
