...
export default function ManagerDashboard({ executive = false }: { executive?: boolean }) {
  ...
  const liveStatuses = useMemo(() => {
    const defs = [
      ...
      { key: 'absent', label: 'Absent', color: '#fca5b4' },
    ];
  ...
  return <div className="space-y-5 p-4 sm:p-6">
    ...
    <section className="grid grid-cols-1 gap-4 2xl:grid-cols-[0.85fr_1.15fr]">
      <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><h2 className="font-bold text-gray-900">Today&apos;s Attendance Overview</h2><div className="mt-7 flex flex-col items-center gap-7 sm:flex-row sm:justify-around"><div className="grid h-44 w-44 place-items-center rounded-full" style={{ background: donut }}><div className="grid h-28 w-28 place-items-center rounded-full bg-white text-center"><div><p className="text-2xl font-bold">{total}</p><p className="text-xs text-gray-500">Total</p></div></div></div><div className="w-full max-w-xs space-y-3">{[['Present',stats.present,'bg-emerald-500'],['Absent',stats.absent,'bg-rose-400'],['On Leave',stats.leave,'bg-amber-500'],['Not Marked',Math.max(0,total-stats.present-stats.absent-stats.leave),'bg-gray-300']].map(([label,value,color]) => <div key={String(label)} className="flex items-center justify-between border-b border-gray-100 pb-2 text-sm"><span className="flex items-center gap-2"><i className={`h-3 w-3 rounded-full ${color}`}/>{label}</span><b>{value as number} ({percent(value as number)}%)</b></div>)}</div></div></div>
        ...
        <div className="mt-5 space-y-4">
          {deptStats.length ? deptStats.map(d => (
            <div key={d.name}>
              ...
              <div className="flex h-2.5 w-full gap-0.5 overflow-hidden rounded-full bg-gray-100">
                <div className="rounded-full bg-emerald-500 transition-all" style={{ width: `${(d.present / d.total) * 100}%` }} />
                <div className="bg-amber-400 transition-all" style={{ width: `${(d.leave / d.total) * 100}%` }} />
              </div>
            </div>
          ...
          <div className="flex items-center gap-2">
            <div className="relative h-14 w-14">
              <svg className="h-14 w-14 -rotate-90" viewBox="0 0 36 36">
                ...
                <circle cx="18" cy="18" r="15.5" fill="none" stroke="#10b981" strokeWidth="4" strokeLinecap="round"
