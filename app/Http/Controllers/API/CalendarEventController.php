...
class CalendarEventController extends Controller
{
    ...
    public function destroy(Request $request, CalendarEvent $calendarEvent): JsonResponse { abort_unless(in_array($request->user()->role, ['ceo','manager','tl']), 403); $calendarEvent->delete(); return response()->json(['message' => 'Agenda deleted.']); }
    ...
    private function format(CalendarEvent $event): array { return ['id'=>$event->id,'title'=>$event->title,'date'=>$event->event_date->format('Y-m-d'),'time'=>$event->event_time ?? '', 'type'=>$event->type,'location'=>$event->location ?? '', 'note'=>$event->note ?? '']; }
}
